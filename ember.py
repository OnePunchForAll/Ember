...
def local_module(name):
    if name not in ('algebra','algebra_check','word_series','word_check','campaign','recurrence','recurrence_check','invariant','invariant_check','invariant_map','obligations','recursive','recursive_check','source_episode','apex','apex_check','pyramid','lexicon','lexicon_check','movebench','agent'): raise Refused('unknown owned module')
...
def capabilities():
    return {'status':'CAPABILITIES','api_version':'ember.task.v1','runtime_version':VERSION,
        ...
        'language':{'operators':'python -I -B -X utf8 ember.py --move-bench executes every operator and reports observed directions',
                    'modules':['ops_seq','ops_poly','ops_orbit','ops_egypt','ops_arith','ops_word','ops_matrix','ops_collatz'],
                    'checker':'lexicon_check.py admits every claim; operators only propose, transfer or refute'},
...
def main():
    ...
    if args.move_bench:
        bench=local_module('movebench').run()
        print(json.dumps(bench,indent=2)); return 0 if bench['ok'] else 2
    if args.task is None: parser.error('a task file, --capabilities, --pyramid or --move-bench is required')
    try:
